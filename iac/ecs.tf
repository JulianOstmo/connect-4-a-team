...
resource "aws_ecs_task_definition" "connect-4-a-team" {
...
data "aws_ecs_task_definition" "main" {
  task_definition = aws_ecs_task_definition.connect-4-a-team.family
}

resource "aws_ecs_service" "aws-ecs-service" {
  name                 = "${var.app_name}-${var.app_environment}-ecs-service"
  cluster              = aws_ecs_cluster.aws-ecs-cluster.id
  task_definition      = "${aws_ecs_task_definition.connect-4-a-team.family}:${max(aws_ecs_task_definition.connect-4-a-team.revision, data.aws_ecs_task_definition.main.revision)}"
  scheduling_strategy  = "REPLICA"
  desired_count        = var.connect-4-a-team-container-count
